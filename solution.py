...
def parse_data(df,logodds,logoddsPA):
    ...
    #Explanation of Change: delete the features of DayOfWeek and PdDistrict, for they are less relatively important.
...
def build_and_fit_model(X_train,y_train,X_test=None,y_test=None,hn=32,dp=0.5,layers=1,epochs=1,batches=64,verbose=0):
    ...
    Y_train=np_utils.to_categorical( y_train.cat.rename_categories( range( len( y_train.unique() ) ) ), len( y_train.unique() ) )  
